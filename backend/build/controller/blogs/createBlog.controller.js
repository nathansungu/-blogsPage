...
const createBlog = (0, asyncHandler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { imgUrl, title, synopsis, content } = yield blogs_schems_1.createBlogSchema.parseAsync(req.body);
    const user_id = req.user.id;
    const newBlog = yield client.posts.create({
        data: { user_id: user_id, imageUrl: imgUrl, title, synopsis, content },
    });
    const { isDeleted, updatedAt } = newBlog, others = __rest(newBlog, ["isDeleted", "updatedAt"]);
